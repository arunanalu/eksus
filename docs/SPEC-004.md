# SPEC-004 — Viabilidade da integração do Projeto Exus com uma demo jogável

> **Status:** proposta técnica para decisão e implementação
>
> **Data da pesquisa:** 19 de julho de 2026
>
> **Base mínima:** [SPEC-001 — firmware háptico de uma zona](SPEC-001.md),
> [SPEC-002 — firmware multi-zona](SPEC-002.md) e
> [SPEC-003 — BLE e OTA](SPEC-003.md).
>
> **Relacionada, mas independente:** a SPEC-002 decide zonas e segurança no
> firmware; a SPEC-003 entrega os transportes BLE e OTA. A Bridge pode nascer e
> ser testada com a zona única da SPEC-001 e USB, depois consumir `BleTransport`.
>
> **Transporte:** a Bridge deve suportar USB Serial e BLE da SPEC-003 por uma
> interface comum; OTA permanece responsabilidade do firmware/CLI da SPEC-003.
>
> **Escopo:** software entre uma experiência jogável em PC e a máscara Exus por
> USB Serial ou BLE; não substitui validação elétrica, ergonômica, ética ou de
> segurança.
...
6. **Bluetooth fora desta entrega:** a SPEC-003 define BLE/GATT, segurança, reconexão, pacotes e testes. A SPEC-004 apenas evita acoplamento que impeça essa integração.
...
| Firmware Exus | ESP32-C3 | Receber comandos, impor limites finais de segurança e controlar os DRV2605L/ERM/LRA. | MVP de uma zona já implementado; precisa evoluir. |
...
│   ├── SPEC-003.md                   # transporte Bluetooth LE e OTA
│   ├── SPEC-004.md                   # integração jogável e Exus Bridge
...
| `/exus-bridge/README.md` | finalidade da Bridge; instalação Python; execução em modo simulado, USB Serial e BLE quando o `BleTransport` da SPEC-003 estiver disponível; seleção de dispositivo; configuração da tabela háptica; logs; protocolo; testes e empacotamento. |
...
### 7.2. Restrição arquitetural para a SPEC-003
...
O ESP32-C3 oferece **Bluetooth Low Energy (BLE)**, mas não Bluetooth Classic. A SPEC-003 define serviço GATT, biblioteca compatível com Arduino/ESP-IDF, segurança e comportamento de desconexão. A Bridge apenas consome esse transporte.

Bluetooth transmite **dados, não energia**. A SPEC-003 separa a substituição do cabo de dados do eventual projeto de bateria. Uma versão realmente sem cabo continuará exigindo especificação elétrica e mecânica própria.
...
### 8.4. Requisitos entregues à SPEC-003

Esta seção registra restrições para a SPEC-003 e **não autoriza nem exige implementar BLE nesta SPEC**. O conteúdo lógico de `H/A/N/X/K/Q` deverá permanecer equivalente nos dois transportes, evitando protocolos com comportamentos incompatíveis.
...
| Enviar comando | escrever linha/pacote na Serial | escrever na característica `command` |
| Receber ACK/status | ler da Serial | receber `response`/`status` por indication/notification |
...
Pontos que a SPEC-003 decide e versiona:

- `Exus Haptic Service`: UUID próprio e versionado;
- `command`: PC escreve comandos hápticos, heartbeat e emergência;
- `response` e `status`: ESP32 indica ACK/NACK e notifica estado;
- `device-info`: leitura de versão de firmware, número de zonas e capacidades;
- `emergency`: escrita de parada imediata; o estado é notificado por `status`.
...
Para uma apresentação pública, o ESP32 não deverá aceitar comandos de qualquer computador próximo. Pareamento/bonding, identificação do cliente permitido e reconexão são critérios da SPEC-003. Mesmo com criptografia do BLE, os tetos de segurança permanecerão no firmware.
...
| **SPEC-003:** BLE sofre interferência ou sai de alcance | atraso, perda de pacote ou desconexão | heartbeat, watchdog local, teste no ambiente real e fallback por USB. |
| **SPEC-003:** outro PC encontra a máscara por BLE | comandos não autorizados | pareamento/bonding, cliente permitido e limites sempre locais no firmware. |
| **SPEC-003:** pacote BLE excede o MTU útil | fragmentação ou comando truncado | pacote binário curto ou protocolo explícito de fragmentação/remontagem. |
| **SPEC-003:** BLE é confundido com alimentação sem fio | protótipo continua preso ao USB ou usa bateria improvisada | separar teste de dados BLE do projeto formal de bateria/regulação/proteção. |
...
Esta SPEC possui um núcleo concluível com o hardware atual e um ponto posterior de integração multi-zona. A [SPEC-002](SPEC-002.md) possui roadmap próprio para multiplexadores, drivers e zonas; portanto, nenhuma das duas precisa bloquear a outra. Bluetooth não aparece como fase: é planejado e aceito exclusivamente pela SPEC-003.

| Trilha/marco | Responsabilidade | Dependência | O que será provado |
|---|---|---|---|
| **A. Bridge + demo, uma zona via USB** | SPEC-004 | SPEC-001 | jogo, Bridge e firmware funcionam ponta a ponta com padrões temporais no hardware atual |
...
| **C. Integração multi-zona via USB** | encontro entre SPEC-002 e SPEC-004 | trilhas A e B | eventos direcionais da demo selecionam IDs lógicos de zonas reais |

Se a SPEC-004 for desenvolvida primeiro, a trilha A entrega uma demo jogável completa em modo de uma zona e a trilha C fica adiada. Se a SPEC-002 terminar primeiro, suas zonas são testadas pela Serial e a trilha C pode começar assim que a Bridge entender o contrato de capabilities.
...
**Núcleo da SPEC-004 concluído quando:** pelo menos quatro eventos chegam por jogo → UDP → Bridge → USB Serial → ESP32 e produzem padrões distintos no hardware atual; perda de cabo ou Bridge sempre para efeitos contínuos. A ausência da SPEC-002 não bloqueia este aceite.
...
- Executar por USB Serial. Uma repetição sem fio pertence à SPEC-003.
...
- [ ] A indisponibilidade da SPEC-002 não impede a demonstração e o aceite da SPEC-004 em modo de uma zona.
...
5. **SPEC-002 e SPEC-004 são trilhas independentes.** A Bridge deve funcionar com a zona única atual; o firmware multi-zona deve funcionar por Serial sem jogo; o aceite integrado acontece quando ambas estiverem prontas.
...
8. **A Exus Bridge é software novo; Bluetooth não faz parte desta entrega.** A interface de transporte deve permitir consumir o BLE da SPEC-003 sem alterar jogo, eventos ou tabela háptica.
...
11. **A SPEC-003 define Bluetooth.** Ela define GATT, pacotes, segurança, reconexão, OTA, regressão de uma/múltiplas zonas e a separação entre dados sem fio e alimentação portátil.
