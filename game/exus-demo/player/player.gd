...
func shoot() -> void:
	...
	if player_id == multiplayer.get_unique_id():
		ExusEventBus.oneshot("weapon_fire", 0.45, 110, 0.0, "player_weapon")


func receive_damage(source_position: Vector3) -> void:
	# O inimigo fornece a origem real; a conversao para azimute fica centralizada no Event Bus.
	var azimuth: Variant = ExusEventBus.azimuth_from_positions(global_position, global_transform.basis, source_position)
	ExusEventBus.oneshot("damage", 0.72, 180, azimuth, "red_robot_laser")
