...
@onready var exus_demo_director: Node = $ExusDemoDirector
...
func spawn_robot(spawn_point) -> void:
	...
	robot.exploded.connect(exus_demo_director.robot_exploded.bind(robot))
	robot.add_to_group("exus_threat")
