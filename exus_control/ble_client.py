"""Cliente BLE assíncrono compartilhado pelos pontos de entrada do Exus."""
...
try:  # A ponte simulada e seus testes não exigem driver/radio BLE instalado.
    from bleak import BleakClient, BleakScanner
    from bleak.exc import BleakError
except ModuleNotFoundError:  # pragma: no cover - exercitado em PCs sem BLE.
    BleakClient = BleakScanner = None

    class BleakError(RuntimeError):
        pass
...
def _name(device, advertisement_data=None) -> str:
    return (advertisement_data.local_name if advertisement_data and advertisement_data.local_name else device.name) or ""
...
class ExusBleClient:
    def __init__(self, device: ExusDevice, *, client_factory=BleakClient):
    ...
    async def disconnect(self) -> None:
        for uuid in (RESPONSE_UUID, STATUS_UUID):
            try: await self._client.stop_notify(uuid)
    ...
    def _received_response(self, _, data: bytearray) -> None:
        ...
        if len(pieces) < 2 or pieces[0] not in {"A", "N"}: return
        try: sequence = int(pieces[1])
        except ValueError: return
        future = self._responses.pop(sequence, None)
        if future and not future.done(): future.set_result(text)

    def _received_status(self, _, data: bytearray) -> None:
        self.status = bytes(data).decode("utf-8", errors="replace").strip()

    async def info(self) -> str:
        return bytes(await self._client.read_gatt_char(DEVICE_INFO_UUID)).decode("utf-8", errors="replace")

    async def command(self, command: str, timeout: float = 5.0) -> str:
        if not self.connected: raise RuntimeError("O Exus não está conectado.")
...
def parse_capabilities(reply: str) -> list[int]:
    ...
    if not raw: return []
    try: return [int(value) for value in raw.split(",")]
    except ValueError: return []
